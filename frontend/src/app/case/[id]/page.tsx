    ...
    return (
      <main className="min-h-screen p-8 bg-slate-950 text-slate-200 flex flex-col items-center justify-center font-mono">
        ...
        <p className="text-slate-500 mb-8">
          The requested forensic record [{params.id}] does not exist or has been
          purged.
        </p>
        <Link
          href="/"
          className="px-6 py-2 border border-slate-700 hover:bg-slate-800 rounded-full transition-colors"
        >
      ...
      <div className="max-w-4xl mx-auto">
        <header className="flex justify-between items-end mb-12 border-b border-slate-800 pb-8">
          ...
          <div className="flex gap-4">
            ...
            <Link
              href="/"
              className="px-6 py-2 border border-slate-700 hover:bg-slate-800 rounded-full font-mono text-[10px] transition-colors uppercase tracking-widest flex items-center"
            >
              New Scan
            </Link>
          </div>
        ...
        <div className="space-y-12">
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div
              className={`flex-1 p-6 rounded-3xl border-2 ${
                caseData.verdict === "AI-Generated"
                  ? "bg-red-950/10 border-red-900/50"
                  : caseData.verdict === "AI-Enhanced/Manipulated"
                    ? "bg-orange-950/10 border-orange-900/50"
                    : "bg-emerald-950/10 border-emerald-900/50"
              }`}
            >
              <span className="text-[9px] uppercase font-black text-slate-600 block mb-2 tracking-[0.2em]">
                Archived Verdict
              </span>
              <h2
          ...
          <div className="mt-12">
            <h3 className="text-sm font-black uppercase tracking-widest border-b-2 border-slate-800 pb-2 mb-6 text-cyan-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {caseData.original_image_url && (
                <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl flex flex-col items-center">
                  <div className="w-full aspect-square bg-slate-950 flex items-center justify-center rounded-xl overflow-hidden mb-4 border border-slate-800">
                    <img
                      src={caseData.original_image_url}
                      alt="Original Evidence"
                      className="max-w-full max-h-full object-contain"
                    />
                  </div>
                  <p className="font-mono text-xs font-bold text-slate-300 tracking-widest uppercase">
                    Exhibit A: Original Image
                  </p>
                </div>
              )}
              ...
              {caseData.evidence_images?.map((img: any, idx: number) => (
                <div
                  key={idx}
                  className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl flex flex-col items-center"
                >
